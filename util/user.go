...
func GetDataRangeIDs(userID int) []int {
	departmentIDs := GetDepartmentIDs(userID)
	var dataRangeIDs []int
	global.DB.Model(&model.DepartmentAndDataRange{}).Where("department_id in ?", departmentIDs).
		Select("data_range_id").Find(&dataRangeIDs)
	dataRangeIDs = RemoveDuplication(dataRangeIDs)
	return dataRangeIDs
}

// GetDepartmentIDs 获得所有的组织id(含子组织)
func GetDepartmentIDs(userID int) []int {
	var organizationIDs []int
	global.DB.Model(&model.DepartmentAndUser{}).Where("user_id = ?", userID).
		Select("department_id").Find(&organizationIDs)
	for i := range organizationIDs {
		res := getInferiorDepartmentIDs(organizationIDs[i])
...
func getInferiorDepartmentIDs(departmentID int) []int {
	var organizationIDs []int
	global.DB.Model(&model.Department{}).Where("superior_id = ?", departmentID).
...
// deprecated
// GetDepartmentIDsOld 获取当前用户所属的所有部门id
//func GetDepartmentIDsOld(userID int) []int {
//	//设置所属部门id数组
