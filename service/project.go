...
func (projectService) Get(disassemblyID int) response.Common {
...
func (projectService) Create(paramIn *dto.DisassemblyCreateOrUpdateDTO) response.Common {
...
func (projectService) CreateInBatches(paramIn []dto.DisassemblyCreateOrUpdateDTO) response.Common {
	//对dto进行清洗，生成dao层需要的model
	var paramOut []model.Disassembly
	//把dto的数据传递给model，由于下面的结构体字段为指针，所以需要进行处理
	for i := range paramIn {
...
// 其次是需要清洗
func (projectService) Update(paramIn *dto.DisassemblyCreateOrUpdateDTO) response.Common {
	var paramOut model.Disassembly
...
func (projectService) Delete(disassemblyID int) response.Common {
...
func (projectService) List(paramIn dto.DisassemblyListDTO) response.List {
	//生成sql查询条件
