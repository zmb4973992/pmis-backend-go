...
func (p *ProgressRouter) InitProgressRouter(param *gin.RouterGroup) {
	...
	progressRouter.GET("/:progress-snow-id", controller.Progress.Get)       //获取进度详情
	progressRouter.POST("", controller.Progress.Create)                     //新增进度详情
	progressRouter.PATCH("/:progress-snow-id", controller.Progress.Update)  //修改进度详情
	progressRouter.DELETE("/:progress-snow-id", controller.Progress.Delete) //删除进度详情
