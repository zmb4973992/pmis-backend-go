...
func (e *ErrorLogRouter) InitErrorLogRouter(param *gin.RouterGroup) {
	...
	errorLogRouter.GET("/:error-log-snow-id", controller.ErrorLog.Get)       //获取错误日志详情
	errorLogRouter.POST("", controller.ErrorLog.Create)                      //新增错误日志
	errorLogRouter.PATCH("/:error-log-snow-id", controller.ErrorLog.Update)  //修改错误日志
	errorLogRouter.DELETE("/:error-log-snow-id", controller.ErrorLog.Delete) //删除错误日志
