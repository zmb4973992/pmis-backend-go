...
func (c *DictionaryTypeRouter) InitDictionaryTypeRouter(param *gin.RouterGroup) {
	...
	dictionaryTypeRouter.GET("/:dictionary-type-id", controller.DictionaryType.Get)                                  //获取字典类型
	...
	dictionaryTypeRouter.PATCH("/:dictionary-type-id", middleware.OperationLog(), controller.DictionaryType.Update)  //修改字典类型
	dictionaryTypeRouter.DELETE("/:dictionary-type-id", middleware.OperationLog(), controller.DictionaryType.Delete) //删除字典类型
