...
func ImportForecastedExpenditure(userId int64) error {
	...
	err := global.DB.Where("name = 'tabFukuan视图中不要导入的记录'").
		First(&tempName).Error
	if err != nil {
		param := service.ErrorLogCreate{
			Detail: "dictionaryType表中找不到”tabFukuan视图中不要导入的记录“这个名称",
		}
		param.Create()
		return err
	}
	...
	global.DBForLvmin.Table("tabFukuan").
		Where("F7117 > 0").
	...
	if err != nil {
		...
		if err != nil {
			param := service.ErrorLogCreate{
				Detail: "在dictionary_detail表中找不到”预测“这个名称(dictionaryType为：收付款的种类)",
			}
			param.Create()
			return err
		}
	...
	for i := range records {
		if i > 0 && i%1000 == 0 {
			...
			fmt.Println("已处理", i, "条预测付款记录，当前进度：100 %")
		}
